...
class BPETokenizer:
    ...
    def str2bytes(self, s: str) -> list[bytes]:
        """
            ...
            for part in re.split(f"({pattern})", chunk):
                if part in special_tokens:
                    ...
                    for match in re.finditer(self.PAT, part):
                        word = match.group(0)
                        token = tuple(self.str2bytes(word))
                        word_count[token] += 1
        return word_count
    
    def init_pair_count(self, word_count: dict[list[bytes], int]) -> dict[tuple[bytes, bytes], int]:
        """
    ...
    def merge_pairs(self,
                    ...
                    word_count: dict[list[bytes], int],
                    vocab_size: int
    ...
    def train_bpe(self,
        ...
        # Step 4. Merge pairs
        vocab, merges = self.merge_pairs(vocab, pair_count, word_count, vocab_size)
        return vocab, merges
...
if __name__ == "__main__":
    ...
    input_path = "test.txt"
    vocab_size = 263
    vocab, merges = tokenizer.train_bpe(input_path, vocab_size, ["<|endoftext|>"])
